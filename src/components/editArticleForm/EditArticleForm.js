...
import { updateUserArticle, getCurrentArticle } from "../store/slicers/articleSlicer";
...
const EditArticleForm = () => {
    ...
    const { title, description, tags, body, slug } = useSelector(state => state.article);
        ...
        <form className="form-article" onSubmit={handleSubmit(onSubmitHandler)}>
            ...
            <div className="form__wrap">
                <div>
                    ...
                    <input {...register("title")} type="text" id="title" placeholder="Title" />
                    {errors.title && <p className="error">{errors.title.message}</p>}
                </div>
                <div>
                    <label htmlFor="description">Short description</label>
                    <input {...register("description")} type="text" id="description" placeholder="Short description" />
                        ...
                        {fields.map((item, index) => (
                            <li key={item.id} className='Tags-flex'>
                                ...
                                {errors.tags?.[index]?.Tag && (
                                 ...
                                 <Button danger style={{ fontWeight: "400", fontSize: "18px", padding: "8px 12px", height:"37.27px", width: "118px" }} onClick={() => remove(index)}>Delete</Button>
                            </li>
                        ))}
                    </ul>
                    {errors.tags?.message && <p className="error">{errors.tags.message}</p>}
                     <Button style={{ fontWeight: "400", fontSize: "18px", padding: "8px 12px", height:"37.27px", width: "118px", marginBottom:"5px" }}  color="primary" variant="outlined"
                        type="button"
                        onClick={() => append({ Tag: "" })}
                    >
                        Add Tag
